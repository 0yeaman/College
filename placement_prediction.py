...
def main():
    ...
    # --- Voting Classifier ---
    print("\n--- Voting Classifier ---")
    # Using the best Random Forest model from tuning and default Logistic Regression and Decision Tree
    # We need to ensure the estimators for VotingClassifier are the actual model objects, not full pipelines
    # if we are applying preprocessing separately or if the VotingClassifier itself is part of a larger pipeline.
    # For simplicity here, we'll define new pipelines for the base estimators if they were tuned,
    # or use the previously defined ones for default.

    # Re-extracting the preprocessor for clarity if needed, or just use it.
    # The base estimators for VotingClassifier in a pipeline should be ('name', model_object)
